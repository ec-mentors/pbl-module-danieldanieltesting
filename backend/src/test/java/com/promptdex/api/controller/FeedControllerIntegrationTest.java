...
public class FeedControllerIntegrationTest {
    ...
    private User userOne; // The user making the request
    private User userTwo; // The user being followed
    private User userThree; // The user NOT being followed
    ...
    void setUp() {
        ...
        // Save users before creating relationships
    ...
    // Helper method to create prompts with specific authors and timestamps
    ...
    void getUserFeed_whenFollowingUsers_returnsOnlyFollowedUsersPrompts() throws Exception {
        // GIVEN: userOne follows userTwo but not userThree
        userOne.follow(userTwo);
        userRepository.saveAndFlush(userOne);

        // WHEN: userOne requests their feed
        // THEN: The feed should contain the prompt from userTwo, but not userThree
        mockMvc.perform(get("/api/feed"))
    ...
    void getUserFeed_withPagination_returnsCorrectPage() throws Exception {
        // GIVEN: userOne follows userTwo, who has two prompts
        ...
        // WHEN: userOne requests the first page with size 1
        // THEN: It should return only the newest prompt and correct pagination info
        mockMvc.perform(get("/api/feed?page=0&size=1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content", hasSize(1)))
                .andExpect(jsonPath("$.totalElements", is(3))) // 2 new + 1 from setup
                .andExpect(jsonPath("$.totalPages", is(3)))
                .andExpect(jsonPath("$.content[0].prompt.title", is("Newer Prompt"))); // Feed is sorted by date descending

        // WHEN: userOne requests the second page
        // THEN: It should return the older prompt
